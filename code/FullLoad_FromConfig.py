...
if __name__ == '__main__':
    ...
    configFile = open(r"../data/FullLoad.config")
    ConfigFileData = json.load(configFile)
    ## print(ConfigFileData['source']['database'])

    src_db      = ConfigFileData['source']['database']
    trg_db      = ConfigFileData['target']['database']
    meta_db     = ConfigFileData['metadata']['database']

    src_table   = ConfigFileData['source']['tablename']
    trg_table   = ConfigFileData['target']['tablename']
    meta_table  = ConfigFileData['metadata']['tablename']  

    password_from_config = ConfigFileData['password']

    print(f'SourceDB is {src_db}')  
    print(f'TargetDB is {trg_db}')  
    print(f'MetaDB is {meta_db}')  

    print(f'SourceTable is {src_table}')  
    print(f'TargetTable is {trg_table}')  
    print(f'MetaTable is {meta_table}')    
    ...
    """
    host = 'localhost'
    print(f"Password before parse {password_from_config}")
    password = urllib.parse.quote_plus(password_from_config) 
    print(f"Password read from COnfig is {password} ")
